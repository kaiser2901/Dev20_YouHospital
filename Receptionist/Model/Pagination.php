...
class Pagination
{
    ...
    {
        $string = strstr($_SERVER['REQUEST_URI'], '&page');
        $str    = str_replace($string, '', $_SERVER['REQUEST_URI']);
        // nếu trang hiện tại bằng 1 thì trả về null
        if ($this->getCurrentPage() === 1) {
            return;
        } else {
            // còn không thì trả về html code
            return '<li class="page-item"><a class="page-link" href="' . $str . '&' . $this->config['querystring'] . '=' . ($this->getCurrentPage() - 1) . '" >Pre</a></li>';
        }
    }
    ...
    private function getNextPage()
    {
        ...
        if ($this->getCurrentPage() >= $this->gettotalPage()) {
            ...
            // còn không thì trả về HTML code
            return '<li class="page-item"><a class="page-link" href="' . $str . '&' . $this->config['querystring'] . '=' . ($this->getCurrentPage() + 1) . '" >Next</a></li>';
        }
    }
        ...
        if (isset($this->config['full']) && $this->config['full'] === false) {
            ...
            for ($i = ($this->getCurrentPage() - 3) > 0 ? ($this->getCurrentPage() - 3) : 1; $i <= (($this->getCurrentPage() + 3) > $this->gettotalPage() ? $this->gettotalPage() : ($this->getCurrentPage() + 3)); $i++) {
                if ($i === $this->getCurrentPage()) {
                    $data .= '<li class="page-item"><a class="page-link active" href="#" >' . $i . '</a></li>';
                } else {
                    $data .= '<li class="page-item"><a class="page-link" href="' . $str . '&' . $this->config['querystring'] . '=' . $i . '" >' . $i . '</a></li>';
                }
            }
        } else {
            // nếu có thì
